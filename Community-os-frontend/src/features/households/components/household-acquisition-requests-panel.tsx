...
export function HouseholdAcquisitionRequestsPanel() {
  ...
  return <Card><CardHeader><CardTitle>Household requests</CardTitle><CardDescription>Verify property ownership or occupancy before assigning access.</CardDescription></CardHeader><CardContent className="space-y-2">{pending.map((request) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-3"><div className="text-sm"><p className="font-medium">{request.requestedBy ? `${request.requestedBy.firstName} ${request.requestedBy.lastName}` : 'Resident'} · {request.household ? label(request.household) : label(request)}</p><p className="text-xs text-muted-foreground">{request.notes || 'No notes provided'}</p></div><div className="flex gap-2"><StatusBadge status={request.status} /><Button size="sm" onClick={() => review.mutate({ id: request.id, status: 'APPROVED' })}><Check className="mr-1 h-4 w-4" />Approve</Button><Button size="sm" variant="destructive" onClick={() => review.mutate({ id: request.id, status: 'REJECTED', reviewNotes: 'Request could not be verified.' })}><X className="mr-1 h-4 w-4" />Reject</Button></div></div>)}</CardContent></Card>
}
