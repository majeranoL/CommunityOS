...
export default function TenantRegisterPage() {
  ...
  usePageTitle(`Join ${community.displayName}`)

  const sendOtp = useSendOtp({
    onSuccess: (result) => {
      const code = result.data?.devCode
      setDevCode(code ? { email, communityId: community.id, code } : null)
      setSendError(null)
      setResendCooldown(OTP_RESEND_SECONDS)
    },
    onError: (error) => {
      setSendError(
        apiErrorMessage(error, 'Could not send the verification code.'),
      )
    },
  })
      ...
      <div className="w-full max-w-lg space-y-6">
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {step === 0
                ? 'Create your account. It becomes active once an administrator approves it.'
                : step === 1
        ...
        {step === 2 ? (
          <Card>
            <CardContent className="flex flex-col items-center gap-4 py-8 text-center">
              ...
              <Button asChild className="mt-2 w-full">
                <Link to={`/c/${community.slug}/login`}>Go to sign in</Link>
              </Button>
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardHeader>
              <CardTitle>
                {step === 0 ? 'Account details' : 'Verify your email'}
              </CardTitle>
              <CardDescription>
                {step === 0
                  ? `Joining ${community.displayName}. Tell us who you are and how to reach you.`
                  ...
                  {step === 0 ? (
                    <>
                      ...
                      <div className="rounded-lg border bg-muted/40 p-4">
                        ...
                        <FormDescription className="mt-2">
                          Provide at least one of block, lot, unit, or address
                          so we can match your household.
                        </FormDescription>
                      </div>
                    ...
                    <>
                      <div className="space-y-2 rounded-lg border bg-muted/40 p-4">
                        ...
                        {devCode &&
                        devCode.email === email &&
                        devCode.communityId === communityId ? (
                          <div className="mt-3 rounded-md border border-dashed p-3">
                            <p className="text-xs font-medium text-muted-foreground">
                              Email delivery is not configured yet. Enter this
                              code:
                            </p>
                          ...
                          <FormDescription className="mt-2">
                            We&apos;ll email you a 6-digit code to confirm this
