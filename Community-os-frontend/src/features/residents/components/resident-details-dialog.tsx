...
function formatHousehold(
  household: { block: string | null; lot: string | null; unit: string | null; address: string | null } | null,
) {
  if (!household) return null
  return [household.block, household.lot, household.unit, household.address].filter(Boolean).join(', ') || null
}

export function ResidentDetailsDialog({ residentId, open, onOpenChange }: ResidentDetailsDialogProps) {
  const { data: resident, isLoading } = useResident(residentId)
  const updateResident = useUpdateResident()
  const deleteResident = useDeleteResident(() => onOpenChange(false))

  const changeStatus = (status: 'ACTIVE' | 'INACTIVE' | 'MOVED_OUT' | 'DECEASED') => {
    if (!residentId) return
    updateResident.mutate({ id: residentId, input: { status } })
  }

  const unitLabel = formatHousehold(
      ...
      <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-3">
              <Avatar className="h-12 w-12">
                ...
                <AvatarFallback>{initials(resident.firstName, resident.lastName)}</AvatarFallback>
              </Avatar>
              <div>
                ...
                <p className="text-sm text-muted-foreground">{resident.email || resident.phoneNumber || '—'}</p>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-2">
              <Badge variant={resident.status === 'ACTIVE' ? 'success' : resident.status === 'DECEASED' ? 'destructive' : 'muted'}>
                {resident.status}
              </Badge>
              {resident.gender ? <Badge variant="secondary">{toTitleCase(resident.gender)}</Badge> : null}
              {resident.civilStatus ? <Badge variant="secondary">{toTitleCase(resident.civilStatus)}</Badge> : null}
            </div>
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <dd>{resident.household ? resident.household.status : '—'}</dd>
              </div>
              <div>
        ...
        <DialogFooter className="gap-2">
          {resident?.status !== 'ACTIVE' ? (
            <Button variant="outline" onClick={() => changeStatus('ACTIVE')} disabled={updateResident.isPending}>
              Mark active
            </Button>
          ) : null}
          {resident?.status === 'ACTIVE' ? (
          ...
          {resident && resident.status !== 'DECEASED' ? (
            <Button
              variant="outline"
              className="text-destructive hover:text-destructive"
              onClick={() => changeStatus('DECEASED')}
              disabled={updateResident.isPending}
            >
              Mark deceased
            </Button>
          ...
          {resident ? (
            <Button
              ...
              className="text-destructive hover:text-destructive"
              onClick={() => {
                if (window.confirm(`Remove ${resident.firstName} ${resident.lastName}?`)) {
