...
import { financeHouseholdsService, type HouseholdOption } from '@/features/finance/services/finance'
...
function useHouseholdOptions(search: string) {
  return useQuery({
    queryKey: ['households', 'options', search],
    queryFn: () => financeHouseholdsService.options({ search: search || undefined, limit: 25 }),
    placeholderData: (previous) => previous,
  })
}

function formatHousehold(household: HouseholdOption) {
  return [household.block, household.lot, household.unit, household.address]
    .filter(Boolean)
    .join(', ') || 'Unnamed unit'
}
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {selected ? formatHousehold(selected) : 'Select a household…'}
        ...
        <div className="max-h-64 overflow-y-auto p-1">
          {isLoading ? (
            ...
            data.items.map((household) => (
              ...
              >
                <span className="truncate">{formatHousehold(household)}</span>
