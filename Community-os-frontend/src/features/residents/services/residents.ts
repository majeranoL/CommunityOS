...
export const residentsService = {
  async list(params: ListQuery = {}) {
    const { data } = await api.get<ApiEnvelope<ResidentListItem[]>>(
      '/residents',
      { params },
    )
  ...
  async get(id: string) {
    const { data } = await api.get<ApiEnvelope<ResidentDetail>>(
      `/residents/${id}`,
    )
    return data.data
  },
  ...
  async update(id: string, input: UpdateResidentInput) {
    const { data } = await api.put<ApiEnvelope<ResidentDetail>>(
      `/residents/${id}`,
      input,
    )
    return data.data
