...
export class VehiclesController {
  ...
  deactivate(@Request() req: any, @Param('id', ParseUUIDPipe) id: string) {
    return this.vehiclesService.deactivate(req.user.community.id, req.user, id);
  }

  @Post(':id/revalidate')
  revalidate(@Request() req: any, @Param('id', ParseUUIDPipe) id: string) {
    return this.vehiclesService.revalidate(req.user.community.id, req.user, id);
  }

  @Delete(':id')
  remove(@Request() req: any, @Param('id', ParseUUIDPipe) id: string) {
    return this.vehiclesService.remove(req.user.community.id, req.user, id);
  }
}
