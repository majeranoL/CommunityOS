...
- Officer roles added to seed (2026-08-23): three assignable board roles with realistic permission subsets — Vice President (all permissions except community.delete/user.delete/audit.manage), Treasurer (full finance/billing/assessments/payments/reports: 41 perms), Secretary (announcements/events/polls/documents/complaints/residents/households/visitors/facility reservations: 52 perms) — alongside President (151) and Member/Renter (25). Three new demo accounts exercise the role-assignment UI: `lorna.bautista@example.com` (Vice President), `maria.delacruz@example.com` (Treasurer), `carlo.mendoza@example.com` (Secretary). Reseeded local + Railway demo DBs and verified role→permission→user counts on both.
