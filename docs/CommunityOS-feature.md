...
- Deployed: commit `c516790` → Railway deploy `ae6c6cb5` SUCCESS (reconciler idempotent 0/0) at `https://backend-production-c9f3e.up.railway.app`; Vercel serves bundle `index-BR_6D4hm.js` matching local `dist`; `/api` rewrite via Vercel → Railway `/api/health` 200. Criteria revision (unpaid balance ≥ ₱10,000 OR 4+ months behind, both configurable) deployed in a follow-up commit — see `docs/PROGRESS.md` work log.
- Live verification on production (`SEED_DB=false`): feature created via superadmin API (`POST /admin/features` → `good-bad-standing` OPTIONAL, id `050995b4-cd55-40fd-945a-4a4f41b93b1a`) and assigned to the demo community with config `{"delinquencyThresholdMonths":3,"restrictedServices":["facility_reservations"]}`; `/api/features` → 200 (pet-registration + good-bad-standing + config). Months threshold round-trip (while default was 3): `PATCH /admin/features/:id/assignments/:communityId` to threshold 4 → BAD household `108cae0d` (3 months behind) shows GOOD; threshold 2 → BAD; restored 3 → BAD. Revoke round-trip: `DELETE` assignment → `/api/features` drops good-bad-standing while `/api/households` still computes standing at the default 3 (BAD) — disabled feature = no restrictions, default thresholds, UI hidden; re-assign restores both features + config. Criteria-revision live verification (balance threshold + new defaults) appended after deploy. Reservation blocking for the BAD household not live-exercised (no resident login for that household in prod); covered by the unit suite.
