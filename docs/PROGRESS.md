...
### 2026-08-17 — IDEA 5 follow-up: BAD-standing criteria revision (unpaid balance OR months behind)
- Per user: a household is BAD standing when it has **₱10,000+ unpaid balance across any billed assessments** (not just recurring dues) **OR** it is **4+ months behind in recurring dues** (was: months-behind only, default 3).
- **Backend:** `summarizeFinance` + `buildDuesTracker` gained a `badStandingBalanceThreshold = 10000` param; standing is now `outstanding >= badStandingBalanceThreshold || monthsBehind >= delinquencyThresholdMonths`. Defaults moved to `DEFAULT_DELINQUENCY_THRESHOLD_MONTHS = 4` and new `DEFAULT_BAD_STANDING_BALANCE_THRESHOLD = 10000` in `feature.constants.ts`. `financeSummary` (`households.service.ts`) and `duesTracker()` (`assessments.service.ts`) read `config.badStandingBalanceThreshold` (validated number, fallback 10000) and pass both thresholds through.
- **Frontend:** `GoodBadStandingConfig` in `feature-communities-dialog.tsx` gained an "Unpaid balance threshold" input (default 10000) alongside the delinquency threshold; Save writes `{ delinquencyThresholdMonths, badStandingBalanceThreshold, restrictedServices }`.
- **Seed:** demo config now `{ delinquencyThresholdMonths: 4, badStandingBalanceThreshold: 10000, restrictedServices: ['facility_reservations'] }`.
- **Tests:** default-threshold expectations updated 3→4 in `households.service.spec.ts` + `dues-tracker.spec.ts`; new cases — balance-threshold BAD without any overdue months (summarizeFinance + tracker), below-threshold GOOD, custom balance threshold (5000 → BAD, 20000 → GOOD). Suite now 12 suites / 67 tests pass.
- **Verified:** backend `tsc --noEmit` exit 0; eslint 0 errors (pre-existing `any` warnings); jest 67/67. Frontend `tsc --noEmit` exit 0; eslint 0 errors; `npm run build` exit 0 (bundle `index-D_0gsd48.js`).
- **Deploy + live verification:** commit `0b29871` → Railway deploy `e7b0346d` SUCCESS (reconciler idempotent 0/0); Vercel serves bundle `index-D_0gsd48.js` matching local `dist`. Prod assignment updated via `PATCH /admin/features/:id/assignments/:communityId` to the new defaults `{"delinquencyThresholdMonths":4,"badStandingBalanceThreshold":10000,"restrictedServices":["facility_reservations"]}`. Balance criterion proven live on household `108cae0d` (outstanding ₱21,800, 3 months behind): with `badStandingBalanceThreshold` 50000 → GOOD (outstanding below, months 3 < 4); restored 10000 → BAD (outstanding ≥ ₱10,000). Combined with the earlier months-threshold round-trip, both criteria are live-verified. See IDEA 5 checker in `docs/CommunityOS-feature.md`.

### 2026-08-17 — IDEA 4: HOA Expenses & Income Statement (core finance, done)
- **User decisions:** income statement is officers-only unless the President grants `finance.income_statement_view` (granted members see a **read-only** view: funds + expenses, no actions); part of the **core finance module** (not feature-gated); fixed `ExpenseCategory` enum; Overview cards relabeled **Collected / Billed / Net receivable**. No new permission codes were added to `MEMBER_PERMISSIONS`.
- **Backend — Expense model:** `Expense` (title, description, `ExpenseCategory` enum, amount decimal, `ExpensePaymentMethod` enum, expenseDate, payee, referenceNumber, notes, `isImported`, `importBatchId`, `createdById`, soft `deletedAt`) + relations (community, createdBy) + migration `20260817120000_add_expense_model`.
- **Backend — Expenses service/controller:** `nextExpenseNumber()` (EXP-######, padStart 6, skips soft-deleted), create/update (soft-delete guard)/remove (soft delete)/findOne/findAll; `ExpenseQueryDto` `page/limit/search/category/from/to/sortBy` (`expenseNumber|expenseDate|amount|title|createdAt`)/`order`, default sort expenseDate desc; routes gated `finance.expense_create/view/update/delete`.
- **Backend — Income statement:** `GET /finance/income-statement` (`finance.income_statement_view`, `IncomeStatementQueryDto` from/to) via `FinanceTransactionsService.incomeStatement()` → `data { from, to, summary { income = PaymentStatus.VERIFIED, expenses = Expense (deletedAt null), fundBalance = income − expenses, billed = assessment receivables }, categories [{category, amount, count}] sorted amount desc, monthly [{month, income, expenses}], expenses [...] }`. Statement embeds `expenses` so view-only members never call the mutation endpoints.
- **Backend — Permissions:** 7 new codes in `prisma/permissions.ts` — `finance.expense_view/create/update/delete/import/export` + `finance.income_statement_view`; `PermissionsProvisioningService` auto-creates + grants to President per community (reconciler).
- **Backend — Import/export:** `expenses` kind added to preview/confirm/cancel + export (CSV/XLSX) in `finance-import-export` (still gated `finance.import`/`finance.export`); exported expenses marked `isImported` + `importBatchId` + `createdById = batch.importedById`.
- **Backend — Security follow-ups (close-out of the IDEA 4 audit):** (1) `uploads.service` proof-file streaming now 404s for non-officers opening another household's payment proof; (2) `finance-import-export` export scoped to caller's own household for non-officers; (3) `finance/options/households|residents` return only the caller's own household for plain members.
- **Frontend:** `constants/permissions.ts` += 7 finance codes; `types/finance.ts` += `ExpenseCategory`/`Expense`/`CreateExpenseInput`/`UpdateExpenseInput`/`IncomeStatement*` and `ImportKind` `'expenses'`; `validation/finance.ts` += `EXPENSE_CATEGORIES` + `expenseSchema`; `services/finance.ts` += `expensesService` + `incomeStatementService`; `hooks/use-finance.ts` += expense + income-statement hooks/keys (import confirm also invalidates expenses + statement); new `components/expense-form-dialog.tsx` (create/edit, category + payment-method selects, DateTimePicker); `finance-page.tsx` += Income statement + Expenses tabs with permission gating, `ExpensesTab` (search, category filter, DataTable with edit/delete, ConfirmDialog), `IncomeStatementTab`/`View` (Collected/Expenses/Fund balance/Billed cards, category list, monthly table, read-only expense records), Overview relabel; `import-export-panel.tsx` gained `expenses` for import + export.
- **Tests:** `expenses.service.spec.ts` (number generation/increment, create/update/remove scoping, soft-delete handling) + `income-statement.service.spec.ts` (summary math, category ordering UTILITIES 3000 → SECURITY 2000, monthly totals, date range). Fixed unused `BadRequestException` import in `expenses.service.ts`.
- **Verified:** backend `tsc --noEmit` exit 0; eslint 0 errors (pre-existing `any` warnings only); jest **14 suites / 73 tests pass**; `npm run build` clean. Frontend `tsc --noEmit` exit 0; eslint 0 errors (49 pre-existing warnings); `npm run build` exit 0; vitest 4 files / 20 tests pass.
- **Deploy + live verification:** commit `ed7bc4d` pushed → Railway deploy `6c2746f8` SUCCESS (reconciler idempotent 0/0); Vercel serves bundle `index-CIchZUHg.js` matching local `dist`. Live on prod (President login): expense create (EXP-000001) → statement income 4,900 / expenses 8,500 / fundBalance −3,600 / billed 25,500 + MAINTENANCE category row; PATCH amount 8,500→9,000; search + category filter; CSV export with attachment header; import preview 2/2 → confirm (EXP-000002 UTILITIES, EXP-000003 MAINTENANCE, both `isImported=true`) → statement expenses 13,700; delete → −5,600; fresh batch cancel; date-range statement returns embedded expenses; 401 unauthenticated on all new routes; temp Member → 403 on `/expenses`, `/finance/income-statement`, `/expenses/:id` (cleaned up). See IDEA 4 checker in `docs/CommunityOS-feature.md`.

### 2026-08-17 — Dashboard: facilities under maintenance + announcements carousel (done)
- **User decisions:** (1) maintenance widget = count + list of facility names (facility status only, no `/maintenance` query so plain Members never 403); (2) MAINTENANCE facilities are **blocked from new reservations** (was only `CLOSED`); (3) announcements promoted to a prominent full-width **carousel** on the dashboard.
- **Backend:** `dashboard.service.ts` overview payload += `recentAnnouncements` (latest 5 `PUBLISHED`, orderBy `publishedAt` desc, select `id/title/content/coverImageUrl/publishedAt/createdAt` — same pattern as `recentComplaints`). `reservations.service.ts` `validateFacility` now also throws `ConflictException('Facility is under maintenance and cannot be reserved.')` for `FacilityStatus.MAINTENANCE` (next to the existing CLOSED block). New unit test `reservations.service.spec.ts` (MAINTENANCE facility → ConflictException, `reservation.create` never called).
- **Frontend:** `dashboard-page.tsx` — new `AnnouncementsCarousel` (index-based, no new dependency: auto-advance 6s with pause-on-hover + `prefers-reduced-motion` respect, dot indicators, prev/next buttons, external cover images only to avoid auth-gated `/uploads` hotlinks, click opens the existing `AnnouncementDetailDialog`, "View all" → `/app/announcements`) rendered full-width directly under the KPI row (gated `announcementView`); new `FacilitiesUnderMaintenanceCard` (count badge + MAINTENANCE facility rows via `useFacilities({ status: 'MAINTENANCE', limit: 100 })`, `StatusBadge` MAINTENANCE, empty state, "View all" → `/app/facilities`) in the bottom grid which became `lg:grid-cols-3` (Upcoming + Open complaints + Under maintenance). `types/dashboard.ts` += `RecentAnnouncement` + `recentAnnouncements`. `reservation-form-dialog.tsx` facility picker disables MAINTENANCE facilities (label "(under maintenance)"). `use-announcements.ts` mutations also invalidate the `dashboardKeys.overview` query so the carousel stays fresh.
- **Verified:** backend `tsc` exit 0; eslint 0 errors; jest **14 suites / 74 tests pass**; `nest build` clean. Frontend `tsc --noEmit` exit 0; eslint 0 errors (49 pre-existing warnings); `npm run build` exit 0 (bundle `index-BV6Z3WLG.js`); vitest 4 files / 20 tests pass.
- **Deploy + live verification:** commit `0dc92b7` pushed → Railway deploy `f8f0b8f9` SUCCESS; Vercel serves bundle `index-BV6Z3WLG.js` matching local `dist`. Live on prod: `GET /dashboard/overview` returns `recentAnnouncements` (1 published "Christmas Event" with content) + `facilityStatus {AVAILABLE:4, MAINTENANCE:0, CLOSED:0}`; booking-block round-trip — set "Function Room A" → MAINTENANCE → `POST /reservations` → **409 "Facility is under maintenance and cannot be reserved."** → restored to AVAILABLE → `facilityStatus` back to 4/0/0.
